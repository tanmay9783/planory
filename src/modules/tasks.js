...
function createTaskCard(task) {
  ...
  const noteIndicator = linkedNote ? `<span class="task-linked-note-indicator" data-date="${linkedNote.date}" title="Linked Note: ${linkedNote.title || 'Untitled Note'}" style="cursor:pointer; margin-left: 6px; font-size:11px; display:inline-flex; align-items:center;"><svg class="task-svg" viewBox="0 0 24 24" width="11" height="11" fill="none" stroke="currentColor" stroke-width="2" style="display:inline-block; vertical-align:middle;"><path d="m21.44 11.05-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"/></svg></span>` : '';
  ...
  card.innerHTML = `
    <div class="task-card-header">
      <div class="task-card-title-wrap">
        ...
        <div class="task-title" style="display:inline-flex; align-items:center; gap:4px; flex-wrap:wrap;">${task.title} ${noteIndicator}</div>
      </div>
    ...
    <div class="task-card-footer" style="padding-left: 36px;">
      <div class="task-card-time" style="display:flex; align-items:center;">
        <svg class="task-svg" viewBox="0 0 24 24" width="11" height="11" fill="none" stroke="currentColor" stroke-width="2" style="display:inline-block; vertical-align:middle; margin-right:4px;"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg> ${task.startTime ? formatTime(task.startTime) : 'All Day'}
      </div>
      <div class="task-actions">
        <button class="icon-btn focus-pomo-btn" title="Focus session" style="display:inline-flex; align-items:center; justify-content:center; padding:4px;"><svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polygon points="10 8 16 12 10 16 10 8"/></svg></button>
        <button class="icon-btn edit-task-btn" title="Edit" style="display:inline-flex; align-items:center; justify-content:center; padding:4px;"><svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 1 1 3 3L12 15l-4 1 1-4z"/></svg></button>
        <button class="icon-btn delete-task-btn" title="Delete" style="display:inline-flex; align-items:center; justify-content:center; padding:4px;"><svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/><line x1="10" x2="10" y1="11" y2="17"/><line x1="14" x2="14" y1="11" y2="17"/></svg></button>
      </div>
    </div>
  `;
...
function setupDropZones() {
  ...
  // Drop support for Focus Timer sidebar button (#sidebar-nav-timer)
  const sidebarNavTimer = document.getElementById('sidebar-nav-timer');
  if (sidebarNavTimer) {
    sidebarNavTimer.addEventListener('dragover', (e) => {
      e.preventDefault();
      sidebarNavTimer.classList.add('drag-hover-glow');
    });
    
    sidebarNavTimer.addEventListener('dragleave', () => {
      sidebarNavTimer.classList.remove('drag-hover-glow');
    });
    
    sidebarNavTimer.addEventListener('drop', (e) => {
      e.preventDefault();
      sidebarNavTimer.classList.remove('drag-hover-glow');
  ...
  // Drop support for Pomodoro Modal Grower Circle (#pomo-forest-grower)
  const pomoForestGrower = document.getElementById('pomo-forest-grower');
  if (pomoForestGrower) {
    pomoForestGrower.addEventListener('dragover', (e) => {
      e.preventDefault();
      pomoForestGrower.classList.add('drag-hover-glow');
    });
    
    pomoForestGrower.addEventListener('dragleave', () => {
      pomoForestGrower.classList.remove('drag-hover-glow');
    });
    
    pomoForestGrower.addEventListener('drop', (e) => {
      e.preventDefault();
      pomoForestGrower.classList.remove('drag-hover-glow');
